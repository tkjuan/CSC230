...
/*    HTML Strings                                                      */
/************************************************************************/
static const char szRestart[] = 
"<head>\r\n\
<title> HTTP Restart </title>\r\n\
</head>\r\n\
<body>\r\n\
Server Restarting\r\n\
<br>\r\n\
...
GCMD::ACTION ComposeHTMLRestartPage(CLIENTINFO * pClientInfo)
{
   ...
   GCMD::ACTION retCMD = GCMD::WRITE;
    ...
    {
         case HTTPSTART:
            Serial.println("Restart Request Detected");
            pClientInfo->cbWrite = BuildHTTPOKStr(true, sizeof(szRestart)-1, ".htm", (char *) pClientInfo->rgbOut, sizeof(pClientInfo->rgbOut));
            pClientInfo->pbOut = pClientInfo->rgbOut;
            pClientInfo->htmlState = WRITECONTENT;
            break;

         case WRITECONTENT:
             pClientInfo->pbOut = (const byte *) szRestart;
             pClientInfo->cbWrite = sizeof(szRestart)-1;
